...
{
    /// <summary>
    /// 雪花算法。分布式Id,使用了大石头的NewLife.Core中的雪花算法
    /// </summary>
    /// <remarks>
    ...
    {
        public static Snowflake Default { get; } = new Snowflake();
        ...
        private void Init()
        {
            ...
            if (WorkerId <= 0)
            {
                ...
                try
                {
                    var ip =Network.NetworkHelper.GetIPs().FirstOrDefault(ip=>ip.AddressFamily == AddressFamily.InterNetwork && !IPAddress.IsLoopback(ip) && ip.GetAddressBytes()[0] != 169);

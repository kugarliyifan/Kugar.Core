...
namespace Kugar.Core.MultiThread
{
    public static class InterlockedEx
    {
        ...
        public static bool IfThen<T>(ref T value, T @if, InterlockedIfThenCallback<T> caller, object userstate) where T : class
        {
            bool v=false;

            if (Interlocked.CompareExchange(ref value, value, @if) == @if)
            {
                v = true;
                caller(@if, userstate);
            }
    ...
    public delegate void InterlockedIfThenCallback<T>(T value, object userstate);
}
